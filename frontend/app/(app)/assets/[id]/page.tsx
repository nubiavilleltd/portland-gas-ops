...
import { ArrowLeft, Package, AlertCircle, Wrench, History } from "lucide-react";
...
function logEventDescription(log: AssetAssignmentLog): string {
  switch (log.event_type) {
    ...
    case "assigned": return log.to_employee_name ? `Assigned to ${log.to_employee_name}` : "Assigned to a staff member";
    case "transferred": return log.to_employee_name ? `Transferred to ${log.to_employee_name}` : "Transferred to new location";
                ...
                <div className="bg-white border border-brand-border rounded-2xl">
                  <div className="relative h-64 bg-gray-50 flex items-center justify-center rounded-2xl overflow-hidden">
                    {asset.attachment_url
                      ? <Image src={asset.attachment_url} alt={asset.name} fill className="object-contain" />
                      : <div className="flex flex-col items-center gap-2 text-gray-300"><Package size={48} /><p className="text-sm">No image</p></div>}
                  </div>
                </div>
                <div className="bg-white border border-brand-border rounded-2xl">
              ...
              <div className="space-y-5">
                <div className="bg-white border border-brand-border rounded-2xl p-5">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    {asset.location && <div className="flex items-center justify-between"><span className="text-brand-text-secondary">Location</span><span className="font-medium text-brand-text-primary">{asset.location}</span></div>}
                  </div>
                </div>
                ...
                <div className="bg-white border border-brand-border rounded-2xl p-5">
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <button onClick={handlePrint} className="w-full flex items-center justify-center gap-2 px-4 py-2 border border-brand-border text-brand-text-primary text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
                      Print QR Code
                    </button>
                  </div>
                </div>
          ...
          {activeTab === "log" && (
            <div className="bg-white border border-brand-border rounded-2xl">
              ...
              {assignmentLogs.length === 0
                ? <div className="flex flex-col items-center justify-center py-12 text-brand-text-secondary"><History size={28} className="mb-2 text-gray-300" /><p className="text-sm">No activity recorded yet</p></div>
                : (
                        ...
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${LOG_EVENT_COLOURS[log.event_type] ?? "bg-gray-100 text-gray-500"}`}>{capitalize(log.event_type.replace(/_/g, " "))}</span>
                            </div>
