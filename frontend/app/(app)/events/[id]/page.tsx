...
const TYPE_STYLES: Record<string, { pill: string; dot: string }> = {
  "Town Hall": { pill: "bg-[var(--brand-primary-faint)] text-[var(--brand-primary)]",  dot: "var(--brand-primary)" },
    ...
    <IntranetLayout>
      ...
      <div className="max-w-[860px] mx-auto px-4 lg:px-8 py-8">
        ...
        <div className="bg-gray-50 border border-gray-100 rounded-2xl p-6 space-y-4">
          <h2 className="text-sm font-bold text-[var(--brand-secondary)]">Event Details</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-xs text-gray-400 mb-0.5">Date</p>
              <p className="font-medium text-[var(--brand-secondary)]">{day} {monthFull} {year}</p>
            </div>
            <div>
              ...
              {ev.virtual_link ? (
                ...
                <p className="font-medium text-[var(--brand-secondary)]">{ev.location ?? "—"}</p>
              )}
            </div>
            <div>
        ...
        {related.length > 0 && (
          <div className="mt-12 pt-8 border-t border-gray-100">
            <h2 className="text-base font-bold text-[var(--brand-secondary)] mb-5" style={{ fontFamily: "var(--font-mulish, sans-serif)" }}>
              ...
              {related.map((r) => {
                ...
                return (
                  <Link
                    key={r.id}
                    href={`/events/${r.id}`}
                    className="group flex items-center gap-4 bg-white border border-gray-100 rounded-xl px-4 py-3 hover:shadow-md hover:border-[var(--brand-primary)]/20 transition-all"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-[var(--brand-secondary)] group-hover:text-[var(--brand-primary)] transition-colors line-clamp-1">{r.title}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{r.location ?? r.virtual_link ?? "Online"}</p>
                    </div>
                    <ChevronRight size={14} className="text-gray-300 group-hover:text-[var(--brand-primary)] transition-colors shrink-0" />
                  </Link>
                );
              })}
            </div>
