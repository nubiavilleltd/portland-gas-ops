...
import { usePayments, usePaymentsByInvoice, useTotalPaidForInvoice } from "@/lib/modules/payments/hooks/usePayments";
...
export default function OrderDetailPage() {
  ...
  const { invoice, isLoading: invoiceLoading } = useInvoiceByOrderId(id);
  const { trips, isLoading: tripsLoading } = useTrips();
  const { payments } = usePayments();
  const { totalPaid:paymentSummary } = useTotalPaidForInvoice(invoice?.id as string);
  ...
  // ── Derived state — defined after guards ─────────────
  const trip = order.trip_id ? getTripById(trips, order.trip_id) : null;
  // const paymentSummary = getPaymentSummary(payments, invoice?.id);
  const balance = invoice
    ? invoice.total_amount - paymentSummary
    : 0;
  ...
  // ── Confirm handler — inline modal-less action ───────
  async function handleConfirm() {
    ...
    try {
      ...
      // Refetch by re-navigating — simple until we have react-query
      router.refresh();
    } catch (err) {
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2 flex-wrap justify-end">
            ...
            {/* Assign to trip — confirmed + pending */}
            {canAssign && (
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white border border-brand-border rounded-2xl p-6">
          ...
          {order.notes && (
            <div className="mt-5 pt-5 border-t border-brand-border text-sm">
              <p className="text-xs text-brand-text-secondary mb-1">Notes</p>
              <p className="text-brand-text-primary">{order.notes}</p>
            </div>
          )}
        </div>
        ...
        >
          <div className="grid grid-cols-3 gap-5">
            ...
            <InfoRow
              label="Amount Paid"
              value={formatCurrency(paymentSummary)}
            />
            <InfoRow
              label="Balance"
              value={invoice ? formatCurrency(balance) : "—"}
