...
function VendorFormModal({
  initial, onClose, onSubmit, loading, title,
}: {
  initial: FormData; onClose: () => void; onSubmit: (data: FormData) => void; loading: boolean; title: string;
}) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="e.g. Persianas Furniture Limited" className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30" />
            ...
            <div>
              ...
              <select value={form.category} onChange={(e) => set("category", e.target.value)} className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30 bg-white">
            ...
            <div>
              ...
              <input value={form.contact_person} onChange={(e) => set("contact_person", e.target.value)} placeholder="e.g. Emeka Okafor" className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30" />
            </div>
            <div>
              <label className="block text-sm font-medium text-brand-text-primary mb-1">Phone</label>
              <input value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="+234 (0) 800 000 0000" className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30" />
            </div>
            <div>
              <label className="block text-sm font-medium text-brand-text-primary mb-1">Email</label>
              <input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="vendor@example.com" className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30" />
              {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-brand-text-primary mb-1">Address</label>
              <textarea value={form.address} onChange={(e) => set("address", e.target.value)} placeholder="Street, City, State" rows={2} className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30 resize-none" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.bank_name} onChange={(e) => set("bank_name", e.target.value)} placeholder="e.g. First Bank" className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30" />
            </div>
            <div>
              <label className="block text-sm font-medium text-brand-text-primary mb-1">Account Name</label>
              <input value={form.account_name} onChange={(e) => set("account_name", e.target.value)} placeholder="e.g. Persianas Furniture Ltd" className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30" />
            </div>
            <div>
              <label className="block text-sm font-medium text-brand-text-primary mb-1">Account Number</label>
              <input value={form.account_number} onChange={(e) => set("account_number", e.target.value)} placeholder="10-digit account number" className="w-full px-3 py-2 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30" />
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium border border-brand-border rounded-lg hover:bg-gray-50 transition-colors">Cancel</button>
            <button type="submit" disabled={loading} className="px-4 py-2 text-sm font-medium bg-brand-purple text-white rounded-lg hover:bg-brand-purple-dark transition-colors disabled:opacity-50">
...
function VendorCard({ vendor, onEdit, onDelete }: { vendor: Vendor; onEdit: (v: Vendor) => void; onDelete: (v: Vendor) => void }) {
  ...
  return (
    <Link href={`/admin/vendors/${vendor.id}`} className="bg-white border border-brand-border rounded-xl p-5 hover:shadow-md transition-shadow flex flex-col gap-3 group">
      ...
      <div className="flex items-center justify-end gap-2 pt-1 border-t border-brand-border">
        <button onClick={(e) => { e.preventDefault(); onEdit(vendor); }} className="flex items-center gap-1 text-xs text-brand-text-secondary hover:text-brand-purple transition-colors px-2 py-1 rounded hover:bg-purple-50">
          <Pencil size={12} /> Edit
        </button>
        <button onClick={(e) => { e.preventDefault(); onDelete(vendor); }} className="flex items-center gap-1 text-xs text-brand-text-secondary hover:text-red-600 transition-colors px-2 py-1 rounded hover:bg-red-50">
...
const TABLE_COLUMNS: Column<Vendor>[] = [
  { key: "vendor_code", label: "Code", render: (_, v) => v.vendor_code ? <span className="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded text-brand-text-secondary">{v.vendor_code}</span> : <span className="text-brand-text-secondary">—</span> },
  { key: "name", label: "Vendor", render: (_, v) => { const ac = AVATAR_COLOURS[v.category] ?? "bg-gray-500"; return <div className="flex items-center gap-2.5"><div className={`h-8 w-8 rounded-lg shrink-0 flex items-center justify-center text-white font-semibold text-xs ${ac}`}>{v.name.charAt(0).toUpperCase()}</div><span className="text-sm font-medium text-brand-text-primary">{v.name}</span></div>; } },
  { key: "category", label: "Category", render: (_, v) => { const cc = CATEGORY_COLOURS[v.category] ?? "bg-gray-100 text-gray-600"; return <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${cc}`}>{capitalize(v.category.replace(/_/g, " "))}</span>; } },
...
export default function AdminVendorsPage() {
  ...
  function handleCreate(form: FormData) {
    createVendor.mutate({ ...form, category: form.category as VendorCategory }, {
  ...
  function handleUpdate(form: FormData) {
    ...
    updateVendor.mutate({ ...form, category: form.category as VendorCategory }, {
  ...
  const editInitial: FormData = editTarget ? {
    name: editTarget.name ?? "", category: editTarget.category ?? "",
    contact_person: editTarget.contact_person ?? "", phone: editTarget.phone ?? "",
    email: editTarget.email ?? "", address: editTarget.address ?? "",
    bank_name: editTarget.bank_name ?? "", account_name: editTarget.account_name ?? "",
  ...
  return (
    <AppLayout pageTitle="Admin — Vendors">
      <PageHeader
        ...
        action={
          <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2 bg-brand-purple text-white text-sm font-medium rounded-lg hover:bg-brand-purple-dark transition-colors">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div className="flex items-center gap-0.5 bg-white border border-brand-border rounded-lg p-0.5 shrink-0">
          <button onClick={() => setView("card")} title="Card view" className={["p-1.5 rounded-md transition-colors", view === "card" ? "bg-brand-purple text-white" : "text-brand-text-secondary hover:text-brand-text-primary"].join(" ")}><LayoutGrid size={14} /></button>
          <button onClick={() => setView("table")} title="Table view" className={["p-1.5 rounded-md transition-colors", view === "table" ? "bg-brand-purple text-white" : "text-brand-text-secondary hover:text-brand-text-primary"].join(" ")}><Table2 size={14} /></button>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          action={!search ? <button onClick={() => setShowForm(true)} className="px-4 py-2 bg-brand-purple text-white text-sm font-medium rounded-lg hover:bg-brand-purple-dark transition-colors">Add Vendor</button> : undefined}
        />
      ) : (
        <>
          ...
          {view === "card" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {vendors.map((v) => <VendorCard key={v.id} vendor={v} onEdit={setEditTarget} onDelete={setDeleteTarget} />)}
            </div>
          ) : (
            <DataTable columns={TABLE_COLUMNS} data={vendors} emptyMessage="No vendors found." searchable={false} rowHref={(v) => `/admin/vendors/${v.id}`} />
          )}
        </>
      )}

      {showForm && <VendorFormModal title="Add New Vendor" initial={EMPTY_FORM} onClose={() => setShowForm(false)} onSubmit={handleCreate} loading={createVendor.isPending} />}
      {editTarget && <VendorFormModal title="Edit Vendor" initial={editInitial} onClose={() => setEditTarget(null)} onSubmit={handleUpdate} loading={updateVendor.isPending} />}
