  ...
  return (
    <Link href={`/admin/assets/${asset.id}`} className="bg-white border border-brand-border rounded-xl overflow-hidden hover:shadow-md hover:border-brand-purple transition-all group flex flex-col">
      <div className="relative h-40 bg-gray-50 flex items-center justify-center overflow-hidden">
        {asset.attachment_url ? <Image src={asset.attachment_url} alt={asset.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" /> : <Package size={36} className="text-gray-300" />}
      </div>
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <div className="mt-auto pt-2 border-t border-brand-border space-y-1">
          {asset.assigned_to && <div className="flex items-center gap-1 text-xs text-brand-text-secondary"><User size={10} className="shrink-0" /><span className="truncate">{asset.assigned_to}</span></div>}
          ...
          {!asset.assigned_to && !asset.location && <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${conditionStyle}`}>{capitalize(asset.condition)}</span>}
        </div>
...
const TABLE_COLUMNS: Column<Asset>[] = [
  { key: "name", label: "Asset", render: (_, asset) => <div className="flex items-center gap-3"><div className="h-9 w-9 rounded-lg bg-gray-100 flex items-center justify-center shrink-0 overflow-hidden">{asset.attachment_url ? <Image src={asset.attachment_url} alt={asset.name} width={36} height={36} className="object-cover h-full w-full" /> : <Package size={16} className="text-gray-400" />}</div><div className="min-w-0 max-w-[260px]"><p className="text-sm font-medium text-brand-text-primary truncate" title={asset.name}>{asset.name}</p>{asset.serial_number && <p className="text-xs text-brand-text-secondary font-mono">{asset.serial_number}</p>}</div></div> },
  ...
  { key: "assigned_to_name", label: "Assigned To", render: (_, asset) => asset.assigned_to_name ? <span className="text-sm text-brand-text-primary">{asset.assigned_to_name}</span> : <span className="text-brand-text-secondary">—</span> },
...
export default function AdminAssetsPage() {
  ...
  return (
    <AppLayout pageTitle="Admin — Assets">
      ...
      {(!isLoading && (assets.length > 0 || search || activeStatus || activeCategoryId)) && (
