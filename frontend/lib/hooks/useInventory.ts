...
import { getAvailableCount, getAvailableItems, getConsumableStockByProduct, getConsumableStockLevel, getDefaultLocation, getInventoryKPIs, getItemById, getItemByTag, getItemsByCustomer, getItemsByOrder, getItemsByProduct, getItemsByStatus, getLocationById, getMovementsByItem, getMovementsByProduct, InventoryKPIs } from "../modules/inventory/selectors/inventory.selectors";
import { INVENTORY_KEYS } from "../modules/inventory/constants/inventory-query-keys";
...
export function useInventoryItemsByStatus(status: Parameters<typeof getItemsByStatus>[1]) {
    const { items, isLoading, error } = useInventoryItems();
