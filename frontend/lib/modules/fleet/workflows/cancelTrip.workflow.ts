...
export async function cancelTripWorkflow(trip: Trip, reason?: string): Promise<Trip> {
  ...
  // Backend handles: driver/vehicle release, order fulfillment revert,
  // inventory release, trip_order cleanup, audit — all atomic
  return TripsService.cancelTrip(trip.id, reason);
}